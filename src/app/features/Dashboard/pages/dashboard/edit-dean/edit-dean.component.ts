...
@Component({
  selector: 'app-edit-dean',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, ReactiveFormsModule],
  templateUrl: './edit-dean.component.html',
  styleUrls: ['./edit-dean.component.css']
})
export class EditDeanComponent implements OnInit {
  deanForm: FormGroup;
  activeTab = 'basic';
  showToast = false;
  toastMessage = '';
  ...
  ngOnInit() {
    this.loadMediaItems();
    this.aboutService.getDeanInfo().subscribe({
      next: (dean) => {
        if (dean) {
          this.populateForm(dean);
        } else {
          this.showErrorToast('Dean information not found');
          this.router.navigate(['/dashboard/about/dean']);
        }
      },
      error: (error) => {
        console.error('Error fetching dean info:', error);
        this.showErrorToast('Failed to load dean information');
        this.router.navigate(['/dashboard/about/dean']);
      }
    });
  ...
  private populateForm(dean: DeanInfo) {
    this.clearFormArrays();
    this.deanForm.patchValue({
      id: dean.id,
      name: dean.name,
      position: dean.position,
      photo: dean.photo,
      greeting: dean.greeting,
      highlight: dean.highlight,
      callToAction: dean.callToAction
    });

    if (dean.message) {
      dean.message.forEach(msg => {
        this.messageArray.push(this.fb.control(msg, Validators.required));
      });
    }

    if (dean.closing) {
      dean.closing.forEach(closing => {
        this.closingArray.push(this.fb.control(closing, Validators.required));
      });
    }
  }
  ...
  saveDean() {
    if (this.deanForm.valid) {
      ...
      this.aboutService.updateDeanInfo(formValue).subscribe({
        next: () => {
          this.showSuccessToast('Dean information updated successfully');
          setTimeout(() => {
            this.router.navigate(['/dashboard/about/dean']);
          }, 3000);
        },
        error: (error) => {
          console.error('Error updating dean info:', error);
          this.showErrorToast('Failed to update dean information');
        }
      });
    } else {
